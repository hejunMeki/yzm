...
import static com.myfams.copy.ImageProcessUtil.cutImage;
import static com.myfams.copy.ImageProcessUtil.grayDeal;
...
public class ImageProcessUtil {
    //字模路径
    private static String zimoFile = "E:\\tool\\orc\\demo\\ftl.txt";

    static int zz = 0;

    public static void main(String[] args) throws IOException {
        /**
         * http://neris.csrc.gov.cn/csrcfund/IMG
         * https://custody.ebank.cmbchina.com/ccss/cmb/cwf.antiflood.ashx?ran="+System.currentTimeMillis()
         */
//
//        for(int i = 0 ;i < 50;i++){
//			saveCheckCodeImg("http://mf.amac.org.cn/amacreport/IMG!execute.action",
//					HttpUtil.getHttpClient(),"cmb"+i);
//		}
        File file = new File("E:\\tool\\orc\\demo\\run");
        int i = 0;
        for (File fileSub : file.listFiles()) {
            i++;
            zz++;
            new ImageProcessUtil().train(fileSub.getPath());
            //String str=new ImageProcessUtil().distingusshCode(fileSub.getPath(), 20,20,5,4);
            //System.out.println(i+"=="+str+"==="+fileSub.getName());
        }
    }
...
//        HttpPost post = new HttpPost(checkCodeUrltemp);
        HttpGet post= new HttpGet(checkCodeUrltemp);
        try {
            response = httpClient.execute(post);
            InputStream is = response.getEntity().getContent();
            os = new FileOutputStream(new File("E:\\tool\\orc\\demo\\run\\" + type +"checkCode.jpeg"));
     ...
     * @throws IOException
     */
    public static void train(String fileUrl) throws IOException {
        List<BufferedImage> list = process(fileUrl, 20, 20, 4, 4);
        String inStr = fileUrl.substring(fileUrl.lastIndexOf("\\") + 1, fileUrl.indexOf("."));
        StringBuffer bfStr = new StringBuffer();
        int i = 0;
        for (BufferedImage bf : list) {
            //System.out.println(i);
            grayDeal(bf, 120);                                //灰度处理
            bfStr.append(inStr.charAt(i)).append("--");
            getImageCode(bf, bfStr);
            bfStr.append("\r\n");
            i++;
            ImageIO.write(bf, "jpeg", new File("E:\\tool\\orc\\demo\\temp\\train" + zz + i + ".jpeg"));
        }
        createFile(new File(zimoFile), bfStr);
    }
     ...
     * @throws IOException
     */
    public static String distingusshCode(String fileUrl, int width, int height, int charsNum, int charWidth) throws IOException {
        List<BufferedImage> list = process(fileUrl, 20, 20, 5, 4);
        String yanzhengma = "";
//        StringBuffer bfStr = new StringBuffer();
//        //2.0识别
//        if (list.size() > 0) {
//            //2.1读取字模
//            List<String> zimo = readTxtFile(zimoFile);
//            int i = 0;
//            for (BufferedImage bf : list) {
//                grayDeal(bf, 190);
//                i++;
//                ImageIO.write(bf, "jpg", new File("E:\\tool\\orc\\demo\\temp\\distingusshCode" + zz + i + ".jpg"));
//                bfStr.setLength(0);
//                //2.2图片--->特征码
//                getImageCode(bf, bfStr);
...
//        }
     ...
     * @throws IOException
     */
    public static List<BufferedImage> process(String fileUrl, int width, int height, int charsNum, int charWidth) throws IOException {
        Color newColor = new Color(100, 100, 100);
        File srcFile = new File(fileUrl);
        BufferedImage file = ImageIO.read(srcFile);
//        bgmToWhite(file);
//        file=rotateImg(file,360,newColor); 	//旋转25度
        grayDeal(file, 120);                                //灰度处理
//        cleanImage(file,file.getHeight(),file.getWidth());//去除干扰线
//        separate(file,file.getHeight(),file.getWidth());//分割图片
        removeNoise(file, 1);                                //去掉噪点
        ImageIO.write(file, "jpeg", new File("E:\\tool\\orc\\demo\\temp\\images" + zz + ".jpeg"));
        return cutImage(file, width, height, charsNum, charWidth);
    }


    /**
    * Description:将图片的背景切换成白色
    * @author jun.he
    * @date 2019/5/16 0016
    */
    public static void bgmToWhite(BufferedImage bi) throws IOException {
            /**
            ...
            for (int i = minx; i < width; i++) {
                for (int j = miny; j < height; j++) {
                     ...
                     */
                    if(rgb[0]>150 && rgb[1]>130&& rgb[2]>150){
                        /**
                         * 这里是判断通过，则把该像素换成黑色
                         */
                            bi.setRGB(i, j, 255);
...
//                        bi.setRGB(i, j, 0);
             ...
             * 将缓冲对象保存到新文件中
             */
            FileOutputStream ops = new FileOutputStream(new File("E:\\tool\\orc\\demo\\temp\\images" + zz + ".jpg"));
         ...
         * @throws IOException
         */
    public static StringBuffer createFtl(List<BufferedImage> list) throws IOException {
        StringBuffer bfStr = new StringBuffer();
        for (BufferedImage bfImage : list) {
            grayDeal(bfImage, 190);                                //灰度处理
            getImageCode(bfImage, bfStr);                            //取得图片的二值化
            bfStr.append("\r\n");
        }
        return bfStr;
    }
    ...
    private static void createFile(File file, StringBuffer buffer) {
        ...
        PrintWriter p = null;
        try {
            p = new PrintWriter(new FileOutputStream(file.getAbsolutePath(), true));
